...
CCBCreateRoleLayer::CCBCreateRoleLayer()
{
    ...
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(CCBCreateRoleLayer::createRoleError), CREATE_ROLE_ERROR, NULL);
}

void CCBCreateRoleLayer::update(float dt)
{
    int speed = 6;
    if (m_deltaAngle != 0)
    {
        speed *= m_deltaAngle > 0 ? 1 : (-1);
        m_deltaAngle -= speed;
        for (int i = 0; i < 3; i++)
        {
            double x = 0;
            double y = 0;
            if ((m_pRoleAngle[i] % 360 > 0 && speed > 0) || (m_pRoleAngle[i] % 360 >= 0 && speed < 0))
            {
                m_pRoleAngle[i] -= speed;
                x = cos(m_pRoleAngle[i] * M_PI / 180) * m_nX;
                y = sqrt((1 - x * x / (m_nX * m_nX)) * (m_nY * m_nY));
                
            }
            else
            {
                m_pRoleAngle[i] -= 2 * speed;
                x = cos(m_pRoleAngle[i] * M_PI / 180) * m_nX;
                y = -sqrt((1 - x * x / (m_nX * m_nX)) * (m_nY * m_nY));
            }
            int angle = m_pRoleAngle[i];
            if (angle > 90)
            {
                angle = 180 - 90;
            }
            ...
            m_Role[i]->setPosition(ccpAdd(ccp(x, y), m_circlePoint));
            float scale = (y - (-m_nY)) * (1 - 0.6) / (m_nY * 2) + 0.6;
            m_Role[i]->setScale(scale);
            float colorScale = 255 * scale * scale;
            m_Role[i]->setColor(ccc3(colorScale, colorScale, colorScale));
            //CCLog("m_deltaAngle, m_pRoleAngle:%d %d %d %f", m_deltaAngle, m_pRoleAngle[i], m_pRoleAngle[i] % 360, scale);
        }
        if (m_deltaAngle == 0)
        {
            m_pTouch->setEnable(true);
...
void CCBCreateRoleLayer::createRoleError(CCObject* pTarget)
{
//    char str[128] = {0};
//    XYTopLayer::getInstance()->ShowTip(DealErrorCode::getServerErrorMsg(dynamic_cast<CCString*>(pTarget)->intValue(), str));
}

void CCBCreateRoleLayer::setTitle()
{
    CCSpriteFrame* pFrame = NULL;
    int roleType = this->getRoleType();
    if (roleType == 1)
    {
        pFrame = CCSpriteFrame::create("CCBLoginResource/label_flower.png", CCRectMake(0, 0, m_RoleType->getContentSize().width, m_RoleType->getContentSize().height));
        if (m_nLastRoleType != 1)
        {
            this->onDice(NULL, 0);
        }
        
        m_pSprRoleDesc->setDisplayFrame(CCSprite::create("CCBLoginResource/role_flower_desc.png")->displayFrame());
        //m_pInputAccount->setFontColor(ccBLUE);
    }
    else if (roleType == 2)
    {
        pFrame = CCSpriteFrame::create("CCBLoginResource/label_sword.png", CCRectMake(0, 0, m_RoleType->getContentSize().width, m_RoleType->getContentSize().height));
        if (m_nLastRoleType == 1)
        {
            this->onDice(NULL, 0);
        }
        
        m_pSprRoleDesc->setDisplayFrame(CCSprite::create("CCBLoginResource/role_sword_desc.png")->displayFrame());
        //m_pInputAccount->setFontColor(ccYELLOW);
    }
    else if (roleType == 3)
    {
        pFrame = CCSpriteFrame::create("CCBLoginResource/label_spear.png", CCRectMake(0, 0, m_RoleType->getContentSize().width, m_RoleType->getContentSize().height));
        if (m_nLastRoleType == 1)
        {
            this->onDice(NULL, 0);
        }
        
        m_pSprRoleDesc->setDisplayFrame(CCSprite::create("CCBLoginResource/role_spear_desc.png")->displayFrame());
        //m_pInputAccount->setFontColor(ccc3(154, 149, 255));
    }

    m_RoleType->setTexture(pFrame->getTexture());
    m_nLastRoleType = roleType;
}

void CCBCreateRoleLayer::setRoleScaleAndColor(CCSprite* pRole, double x, double y)
{
    float scale = (y - (-m_nY)) * (1 - 0.6) / (m_nY * 2) + 0.6;
    pRole->setScale(scale);
    float colorScale = 255 * scale * scale;
    pRole->setColor(ccc3(colorScale, colorScale, colorScale));
    //CCLog("m_deltaAngle, m_pRoleAngle:%d %d %d %f", m_deltaAngle, m_pRoleAngle[i], m_pRoleAngle[i] % 360, scale);
}
...
CCBCreateRoleLayer::~CCBCreateRoleLayer()
{
    ...
    CC_SAFE_RELEASE_NULL(BtnLeft);
    CC_SAFE_RELEASE_NULL(BtnRight);
    CC_SAFE_RELEASE_NULL(BtnDice);
    
    CC_SAFE_RELEASE_NULL(m_Role0);
    CC_SAFE_RELEASE_NULL(m_Role1);
    CC_SAFE_RELEASE_NULL(m_Role2);
    CC_SAFE_RELEASE_NULL(m_RoleType);
    
    CC_SAFE_RELEASE_NULL(m_pSprRoleDesc);
...
SEL_CCControlHandler CCBCreateRoleLayer::onResolveCCBCCControlSelector(CCObject * pTarget, const char * pSelectorName)
{
    CCB_SELECTORRESOLVER_CCCONTROL_GLUE(this, "onCreate", CCBCreateRoleLayer::onCreate);
    CCB_SELECTORRESOLVER_CCCONTROL_GLUE(this, "onBack", CCBCreateRoleLayer::onBack);
    //CCB_SELECTORRESOLVER_CCCONTROL_GLUE(this, "onLeft", CCBCreateRoleLayer::onLeft);
    //CCB_SELECTORRESOLVER_CCCONTROL_GLUE(this, "onRight", CCBCreateRoleLayer::onRight);
    CCB_SELECTORRESOLVER_CCCONTROL_GLUE(this, "onDice", CCBCreateRoleLayer::onDice);
    return NULL;
}

bool CCBCreateRoleLayer::onAssignCCBMemberVariable(CCObject * pTarget, const char * pMemberVariableName, CCNode * pNode)
{
    ...
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "BtnLeft", CCControlButton*, BtnLeft);
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "BtnRight", CCControlButton*, BtnRight);
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "BtnDice", CCControlButton*, BtnDice);
    
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "Role0", CCSprite*, m_Role0);
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "Role1", CCSprite*, m_Role1);
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "Role2", CCSprite*, m_Role2);
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "RoleType", CCSprite*, m_RoleType);
    
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "spr_role_desc", CCSprite*, m_pSprRoleDesc);
    CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "node_particle", CCNode*, m_pNodeParticle);
    
    return false;
}

void CCBCreateRoleLayer::onNodeLoaded(CCNode * pNode, CCNodeLoader * pNodeLoader)
{
    loadLayer();
}

void CCBCreateRoleLayer::loadLayer()
{
    this->setPosition(ccp((SCREEN_WIDTH - this->getContentSize().width) / 2, (SCREEN_HEIGTH - this->getContentSize().height) / 2));
    
    m_pSprRoleDesc->setZOrder(100);
    ...
    m_circlePoint = ccpAdd(m_circlePoint, ccp(0, -deltaY));
    m_Role0->setPosition(ccpAdd(m_Role0->getPosition(), ccp(0, -deltaY)));
    m_Role1->setPosition(ccpAdd(m_Role1->getPosition(), ccp(0, -deltaY)));
    m_Role2->setPosition(ccpAdd(m_Role2->getPosition(), ccp(0, -deltaY)));
    
    setPrority(-20);
    BtnBack->setTouchPriority(getPrority()-1);
    BtnCreate->setTouchPriority(getPrority()-1);
    //BtnLeft->setTouchPriority(getPrority()-1);
    //BtnRight->setTouchPriority(getPrority()-1);
    m_pLeft = XYBaseTouchSprite::create("public/onePixels.png", getPrority()-1);
    BtnLeft->getParent()->addChild(m_pLeft);
    m_pLeft->setPosition(BtnLeft->getPosition());
    m_pLeft->setScaleX(BtnLeft->getContentSize().width);
    m_pLeft->setScaleY(BtnLeft->getContentSize().height);
    m_pLeft->setTouchEndEvent(this, callfuncN_selector(CCBCreateRoleLayer::onLeft));
    
    m_pRight = XYBaseTouchSprite::create("public/onePixels.png", getPrority()-1);
    BtnRight->getParent()->addChild(m_pRight);
    m_pRight->setPosition(BtnRight->getPosition());
    m_pRight->setScaleX(BtnRight->getContentSize().width);
    m_pRight->setScaleY(BtnRight->getContentSize().height);
    m_pRight->setTouchEndEvent(this, callfuncN_selector(CCBCreateRoleLayer::onRight));
    
    m_pTouch = XYTouchSprite::create("public/onePixels.png", true, getPrority()-2);
    this->addChild(m_pTouch);
    m_pTouch->setPosition(ccp(490, 300));
    m_pTouch->setScaleX(1500);
    m_pTouch->setScaleY(400);
    m_pTouch->setTouchEndEvent(this, callfuncND_selector(CCBCreateRoleLayer::onTouch));
    
    BtnDice->setTouchPriority(getPrority()-3);
    
    m_pEditBoxAccount = XYEditBox::create(CCSize(240, 60), CCScale9Sprite::create());
    m_pEditBoxAccount->setAnchorPoint(ccp(0.5, 0.5));
    m_pEditBoxAccount->setTouchPriority(getPrority()-3);
    m_pEditBoxAccount->setPosition(ccp(0, 0));
    m_pEditBoxAccount->setText("");
    m_pEditBoxAccount->setContentHorizontalAlignment(kHorizontalAlignmentCenter);
    getChildByTag(101)->addChild(m_pEditBoxAccount);
    
    getChildByTag(102)->setZOrder(50);
    getChildByTag(103)->setZOrder(51);
    getChildByTag(104)->setZOrder(51);
    getChildByTag(105)->setZOrder(51);
    getChildByTag(101)->setZOrder(52);
    
    
    BtnCreate->setZOrder(53);
...
int CCBCreateRoleLayer::getRoleType()
{
    for (int i = 0; i < 3; i++)
    {
        if (m_pRoleAngle[i] == 90)
        {
            return m_Role[i]->getTag();
        }
    }
    return 0;
}

void CCBCreateRoleLayer::onCreate(CCObject* pTarget, CCControlEvent event)
{
    if (CommonUtility::getUTF8Length(m_pEditBoxAccount->getText()) < 1 ||  CommonUtility::getUTF8Length(m_pEditBoxAccount->getText()) > 10)
    {
        XYTopLayer::getInstance()->ShowTip("角色名称长度为1-10个字符！");
        return;
    }
    if (checkDirtyWord(m_pEditBoxAccount->getText()))
    {
        XYTopLayer::getInstance()->ShowTip("角色名称非法！");
        return;
    }
    const char* code = "";//m_pInputInviteCode->getText();
    int len = strlen(code);
    if (len != 0)
    {
        for (int i = 0; i < len; i++)
        {
            ...
            {
                XYTopLayer::getInstance()->ShowTip("邀请码必须为全数字！");
                return;
            }
        }
    }
    int tag = this->getRoleType();
    XYHttpLogin::getInstance()->createRole(m_pEditBoxAccount->getText(), "", tag);
}
void CCBCreateRoleLayer::onBack(CCObject* pTarget, CCControlEvent event)
{
    LoginController::sharedInstance()->popLayer();
}
void CCBCreateRoleLayer::onTouch(CCNode* pNode, void* pFlag)
{
    bool* pIsPositive = (bool*)pFlag;
    if (*pIsPositive)
    {
        this->onLeft(NULL);
    }
...
void CCBCreateRoleLayer::onLeft(CCNode* pNode)
{
    m_pTouch->setEnable(false);
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        switch (m_pRoleAngle[i]) {
            case 180:
            case 90:
                m_Role[i]->setZOrder(40);
                break;
            case 0:
                m_Role[i]->setZOrder(30);
                break;
            default:
...
void CCBCreateRoleLayer::onDice(CCObject* pTarget, CCControlEvent event)
{
    char str[128] = {0};
    int first = rand() % m_firstVec.size();
    if (this->getRoleType() != 1)
    {
    ...
    m_pEditBoxAccount->setText(str);
}

void CCBCreateRoleLayer::onRight(CCNode* pNode)
{
    m_pTouch->setEnable(false);
    BtnLeft->setEnabled(false);
    BtnRight->setEnabled(false);
