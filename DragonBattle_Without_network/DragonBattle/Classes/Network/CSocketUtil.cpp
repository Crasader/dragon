...
int CSocketUtil::SetBlockMode(int iSyncState)
{
	int iBlockFlag;
	// Blocking or not
	if ((iBlockFlag = fcntl(m_iSocketID, F_GETFL)) == -1)
	{
		return -1;
	}
...
#endif
				//下面的一句一定要，主要针对防火墙
				getsockopt(m_iSocketID, SOL_SOCKET, SO_ERROR, &error, &len);
...
int CSocketUtil::DataArrival(int T)
{
	...
	timeout.tv_sec = 0;
	timeout.tv_usec = T;
	select(m_iSocketID + 1, &fdsck, NULL, NULL, &timeout);
